...
<script setup lang="ts">
  ...
  const changeFocusDown = (index: number) => {
    ...
    if (cityTipsArr !== null && cityTipsArr[index] instanceof HTMLButtonElement && index <= (cityTipsArr.length - 2)) {
  ...
  const changeFocusUp = (index: number) => {
    ...
    if (searchInput !== null && searchInput instanceof HTMLInputElement && index === 0) {
      searchInput?.focus()
    } else {
      cityTipsArr[index - 1].focus()
    }
