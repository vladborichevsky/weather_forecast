<template>
  <div className="main_window">
    <header-block :city="city" />

    <search-block v-model.trim="city" @getWeatherEmit="getWeather()" />
    ...
    <error-block :errorFetchMes="errorFetchMes" :fetchFail="fetchFail" />

    <tips-city-name :tipsCityName="tipsCityName" @setCityName="setCityName" />

    <!-- как только пользователь запросил данные о погоде - запускается спиннер  -->
    <waiting-spinner v-show="showSpinner" />
    ...
    <start-screen v-show="showStartScreen" />
  </div>
</template>
...
const errorFetchMessages = {
  ...
  fetchFail:
    'We apologize! There was a technical problem with the site. Please try again later :(',
}
...
const computedTipsCityName = computed(() => {
  if (city.value.includes(' ')) {
    ...
    const cityName = city.value[0]?.toUpperCase() + city.value.slice(1)
    return availableCityList.value.filter(item => item.name.includes(cityName))
  }
})
  ...
  () => {
    ...
    if (city.value.length == 0) {
      ...
      window.history.pushState(
        null,
        document.title,
        `${window.location.pathname}?city_name=${city.value}`,
      )
    }
